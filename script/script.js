...
function usarLocalizacao() {
  ...
  navigator.geolocation.getCurrentPosition(
    ...
    function (position) {
      ...
      const vagasEncontradas = centralizarMapa(latitude, longitude, true);

      mostrarToast(`Localização encontrada! ${vagasEncontradas} vagas próximas.`);
    },
...
function criarIconePonto(cor, tamanho) {
  return L.divIcon({
    className: "",
    html: `<div style="width:${tamanho}px;height:${tamanho}px;border-radius:50% 50% 50% 0;transform:rotate(-45deg);background:${cor};box-shadow:0 4px 10px -2px rgba(15,50,110,.45);border:2px solid #fff;"></div>`,
...
function iniciarMapaReal(lat, lng) {
  ...
  centralizarMapa(coordenada.lat, coordenada.lng, false);
...
// Move o mapa para novas coordenadas (sempre dentro do Brasil) e gera vagas simuladas ao redor do ponto real
function centralizarMapa(lat, lng, comAnimacao) {
  const coordenada = garantirCoordenadaNoBrasil(lat, lng);
  ...
  mapaSpoty.setView([coordenada.lat, coordenada.lng], 16, { animate: comAnimacao });
  ...
  marcadoresVagas.forEach(function (marcador) {
    mapaSpoty.removeLayer(marcador);
  });
  marcadoresVagas = [];

  // Marca a posição real (endereço buscado ou localização do usuário)
  marcadorUsuario = L.marker([coordenada.lat, coordenada.lng], {
    icon: criarIconePonto("var(--blue-500)", 16)
  })
    .addTo(mapaSpoty)
    .bindPopup("Você está aqui");

  // Gera vagas simuladas nas proximidades da coordenada real
  const cores = ["var(--green-500)", "var(--yellow-400)", "var(--green-500)", "var(--yellow-400)"];
  const quantidadeMarcadores = Math.floor(Math.random() * 3) + 2;
  const vagasEncontradas = Math.floor(Math.random() * 15) + 3;

  for (let i = 0; i < quantidadeMarcadores; i++) {
    const offsetLat = (Math.random() - 0.5) * 0.012;
    const offsetLng = (Math.random() - 0.5) * 0.012;
    const cor = cores[i % cores.length];

    const marcadorVaga = L.marker([coordenada.lat + offsetLat, coordenada.lng + offsetLng], {
      icon: criarIconePonto(cor, 14)
    })
      .addTo(mapaSpoty)
      .bindPopup("Vaga disponível");

    marcadoresVagas.push(marcadorVaga);
  }

  if (freeCount) {
    freeCount.textContent = vagasEncontradas;
  }
...
function buscarCoordenadasEAtualizarMapa(endereco) {
  ...
  fetch(url)
    ...
    .then(function (dados) {
      ...
      const vagasEncontradas = centralizarMapa(lat, lng, true);

      mostrarToast(`Encontramos ${vagasEncontradas} vagas próximas de ${endereco}.`);
    })
    .catch(function (erro) {
